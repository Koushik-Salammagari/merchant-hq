...
function MessageRow({ message, draft, onDraftChange, onReply, busy }) {
  // A row-level component (not inlined in .map()) so useHighlight — a
  // hook — can be called per row without breaking the Rules of Hooks.
  const highlight = useHighlight("messages", message.id);

  return (
    <div key={highlight?.at ?? "base"} className={`px-6 py-4 ${flashClassName(highlight)}`}>
    ...
    <Panel title="Messages" subtitle={`${messages.length} message${messages.length === 1 ? "" : "s"}`}>
      ...
      {messages.map((message) => (
        <MessageRow
          key={message.id}
          message={message}
          draft={draftById[message.id] ?? ""}
          onDraftChange={(value) => setDraftById((prev) => ({ ...prev, [message.id]: value }))}
          onReply={onReply}
          busy={busyKeys.has(`message:${message.id}`)}
        />
      ))}
    </Panel>
  );
