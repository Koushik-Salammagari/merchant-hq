...
export const api = {
  ...
  async draftMessageReply(messageId, replyText, who) {
    const data = await fetchJson(`/api/messages/${encodeURIComponent(messageId)}`, {
      method: "PATCH",
      body: JSON.stringify({ replyText }),
    });
    upsertMessage(data.message, who);
    return data;
  },
  ...
  async createDiscountCode(percentOff, code, who) {
    const data = await fetchJson("/api/discounts", {
      method: "POST",
      body: JSON.stringify({ percentOff, ...(code ? { code } : {}) }),
    });
    addDiscount(data.discount, who);
    return data;
  },
};
